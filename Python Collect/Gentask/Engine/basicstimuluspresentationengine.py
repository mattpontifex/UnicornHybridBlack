...
class Engine():
    ...
    def __init__(self):
        ...
        self.experimenterkeys = ['space', 'enter', 'return', 'escape', 'a', 'r', 'q', 'p']
    ...
    def start(self):
        ...
        if (self.filenametooltip == ''):
            dlg.addField('Participant ID:', tempid)
        else:
            dlg.addField('Participant ID:', tempid, tip=self.filenametooltip)
        ...
        if not self.quit:
            ...
            if self.expdisp:
                self.experimenternotificationtext = visual.TextStim(self.experimenterwin, text='...', height = 0.05, pos=[-0.95,-0.95], alignHoriz = 'left', color=self.experimentermonitor.forgroundcolor, autoLog=False)
                self.experimenternotificationtext.setAutoDraw(True); self.experimenterwin.flip()
            
            
            if self.participantprogressnotice:
                self.participantprogressnoticetext = visual.TextStim(self.participantwin, text='...', height = 0.05, pos=[-0.95,-0.95], alignHoriz = 'left', color=self.participantmonitor.forgroundcolor, opacity=0.35, autoLog=False)
            ...
            if (len(self.unicorn) > 0):
                ...
                try:
                    ...
                    checktimer = core.CountdownTimer(start=2.0)
            ...
            mouse = event.Mouse(win=self.participantwin) 
            mouse.getPos()
            
            ###########################################
            #####  Function for response checking #####
            ###########################################         
            self.checkkeytimer = core.CountdownTimer(self.refreshrate*0.8) # 80% of refreshrate
            def checkifresponse():
                continuecheck = True
                response = [None,None] # button, time
                self.checkkeytimer.reset() # sets for preestablished time
                while (self.checkkeytimer.getTime() > 0.0) and continuecheck:
                    tc = self.cumulativeTime.getTime() # snag current time
                    # check if button press
                    theseKeys = event.getKeys(keyList=self.experimenterkeys+self.participantkeys+self.mritriggerinputs, timeStamped=self.cumulativeTime)
                    if (len(theseKeys) > 0): #at least one key was pressed
                        response[0] = theseKeys[0][0]
                        response[1] = theseKeys[0][1]
                    else:
                        if self.buttons != None:
                            # see if the mouse is in the hit area
                            if mouse.mouseMoved():
                                for cS in range(len(self.buttons)):
                                    if self.buttonpolygon[cS].contains(mouse):
                                        response[0] = self.buttons[cS].get('buttonCode')
                                        response[1] = tc
                            else:
                                # see if there was a mouse press
                                for cS in range(len(self.buttons)):
                                    if mouse.isPressedIn(self.buttonpolygon[cS]):
                                        response[0] = self.buttons[cS].get('buttonCode')
                                        response[1] = tc
                        ...
                        if not self.quit:
                            ...
                            if self.instructioncard[n].endswith(tuple(image_file_types)): # Check if image
                                ...
                                while (participantstimulus.status != visual.FINISHED) and continueRoutine:
                                    ...
                                    response = checkifresponse() # [key, cumulativeRT]
                                    if (response[0] in ["escape", "q"]): # Check for kill keys
                                        self.quit = True
                                        continueRoutine = False
                                        self.buttonhit(response[0])
                                        break
                                    elif (response[0] in self.experimenterkeys): # Expermenter can always skip
                                        continueRoutine = False
                                        break
            ...
            if not self.quit:
                ...
                if self.expdisp:
                    if self.activedisplaylog:
                        ...
                        self.experimenterbuttonticks = [visual.TextStim(self.experimenterwin, text='|', height = 0.03, pos=[n,0.93], alignHoriz = 'left', color=self.experimentermonitor.forgroundcolor, autoLog=False) for n in ticklist]
                        for n in range(0,len(ticklist)):
                            self.experimenterbuttonticks[n].setAutoDraw(True)
                        self.experimentertrackers = [visual.TextStim(self.experimenterwin, text='stim', height = 0.05, pos=[0,0.9], alignHoriz = 'left', color=self.experimentermonitor.forgroundcolor, autoLog=False) for n in range(0,20)]
                    ...
                    if self.waitcard:
                        ...
                        while continueRoutine:
                            ...
                            if (response[0] in ["escape", "q"]): # Check for kill keys
                                ...
                                continueRoutine = False 
                            elif (response[0] in self.mritriggerinputs): # If a sync pulse was received
                                continueRoutine = False  
                    ...
                    for self.trial in range(1,self.sequencelistL):
                        if not self.quit:
                            ...
                            if (response[0] in ["escape", "q"]): # Check for kill keys
                                ...
                                self.buttonhit(response[0])
                                break
                            elif (response[0] in ["p"]): # Check to see if the task is being paused
                                ...
                                while continueRoutine:
                                    response = checkifresponse() # [key, cumulativeRT]
                                    if (response[0] in self.experimenterkeys): # Expermenter can always end it
                                        break
                                if self.expdisp:
                            ...
                            if self.participantprogressnotice:
                                ...
                                tplace = self.participantprogressnotice[closestidx(numpy.asarray(self.participantprogressnotice),numpy.divide(self.trial, self.sequencelistL))]
                            ...
                            if self.sequencelist[self.trial][self.seqNstimulustype] == '0': # if the stimulus type is an image
                                ...
                                while self.continuetrial:
                                    ...
                                    if (response[0] in ["escape", "q"]): # Check for kill keys
                                        ...
                                        t = (response[1]-self.cumulstimOnTime)
                                        gt = response[1]
                                        if (self.trialkeys == []):
                                            self.trialkeys = response[0]
                                            self.sendtrigger(response[0])
                                            self.trialRT = t
                                            self.trialRTg = gt
                                        self.resparray[self.trial].append([self.trial, response[0], t, gt, 0])
                                        ...
                                        self.updateexperimentermarker(response[0],gt)
                                        self.buttonhit(response[0])
                                        if self.debug:
                                            print('keypress received:', response[0])
                            ...
                            if self.expdisp:
                                ...
                                #self.experimenternotificationtext.setText('Trial %d: %s %s ' % (self.trial, trialcorrect, overallaccuracy)); self.experimenterwin.flip()
                                tplace = round(numpy.multiply(numpy.divide(self.trial, self.sequencelistL),100), 0)
                                self.experimenternotificationtext.setText('%.0f%% Complete.' % (tplace)); self.experimenterwin.flip()
                                    ...
                                    if (response[0] in ["escape", "q"]): # Check for kill keys
                                        ...
                                        if self.triggers:
                                            if self.markfirstresponseonly and (len(self.resparray[self.trial]) > 0):
                                                self.resparray[self.trial].append([self.trial, response[0], ("%.3f" % (numpy.round((t)*1000,3))), ("%.6f" % (numpy.round(gt,6))), 0])
                                            else:
                                                self.sendtrigger(response[0])
                                                self.resparray[self.trial].append([self.trial, response[0], ("%.3f" % (numpy.round((t)*1000,3))), ("%.6f" % (numpy.round(gt,6))), 1])
                                        else:
                                            self.resparray[self.trial].append([self.trial, response[0], ("%.3f" % (numpy.round((t)*1000,3))), ("%.6f" % (numpy.round(gt,6))), 0])
                                        self.updateexperimentermarker(response[0],gt)
                                        self.buttonhit(response[0])
                                            ...
                                            if (response[0] in ["escape", "q"]): # Check for kill keys
                                                ...
                                                if self.triggers:
                                                    if self.markfirstresponseonly and (len(self.feedbackresparray[self.trial]) > 0):
                                                        self.feedbackresparray[self.trial].append([self.trial, response[0], ("%.3f" % (numpy.round((t)*1000,3))), ("%.6f" % (numpy.round(gt,6))), 0])
                                                    else:
                                                        self.sendtrigger(response[0])
                                                        self.feedbackresparray[self.trial].append([self.trial, response[0], ("%.3f" % (numpy.round((t)*1000,3))), ("%.6f" % (numpy.round(gt,6))), 1])
                                                else:
                                                    self.feedbackresparray[self.trial].append([self.trial, response[0], ("%.3f" % (numpy.round((t)*1000,3))), ("%.6f" % (numpy.round(gt,6))), 0])
                ...
                if not self.quit:
                    if self.pauseatend:
                        ...
                        if self.expdisp:
                            ...
                            self.participantnotificationtext = visual.TextStim(self.participantwin, text='Task completed. \nPress any of these keys to exit: %s' % (listofkeys), height = 0.05, pos=[0.5,0], alignHoriz = 'center',  color=self.participantmonitor.forgroundcolor, autoLog=False)
    ...
    def sendtrigger(self, val):
        if self.triggers:
            if (len(self.unicorn) > 0):        
                try:
                    self.UnicornBlack.mark_event(val) # Send trigger 
                except:
                    bolerr = 1
    ...
    def drawbuttons(self):
        if self.buttons != None:
            ...
            self.buttonpolygon = []
            for cS in range(len(self.buttons)):
                self.buttonpolygon.append(visual.Rect(
                    win=self.participantwin, name='polygon',
                    width=self.buttons[cS].get('width'), height=self.buttons[cS].get('height'),
                    ori=0, pos=self.buttons[cS].get('pos'),
                    lineColor=self.buttons[cS].get('lineColor'), lineColorSpace='rgb', lineWidth=self.buttons[cS].get('lineWidth'), 
                    fillColor=self.buttons[cS].get('fillColor'), fillColorSpace='rgb', opacity=self.buttons[cS].get('opacity'), 
                    depth=-1.0, interpolate=True))
            
            self.buttonlabels = []
            for cS in range(len(self.buttons)):
                self.buttonlabels.append(visual.TextStim(win=self.participantwin, name='labels',
                    text=self.buttons[cS].get('buttonText'),
                    font='Arial', bold=True,
                    pos=self.buttons[cS].get('pos'), height=self.buttons[cS].get('textHeight'), wrapWidth=None, ori=0,
                    color=self.buttons[cS].get('textColor'), colorSpace='rgb', opacity=self.buttons[cS].get('textOpacity'),
                    depth=-2.0))
            
            for cS in range(len(self.buttons)):
                self.buttonpolygon[cS].setAutoDraw(True)
                self.buttonlabels[cS].setAutoDraw(True)
    
    def buttonhit(self, buttonCode, duration=.05):
        if buttonCode != None and self.buttons != None:
            for cS in range(len(self.buttons)):
                if self.buttons[cS].get('buttonCode') == buttonCode:
                    self.buttonpolygon[cS].fillColor = '#3D5E73'
            
            for cF in range(int(duration/self.refreshrate)):
                self.participantwin.flip()
            
            for cS in range(len(self.buttons)):
                if self.buttons[cS].get('buttonCode') == buttonCode:
                    self.buttonpolygon[cS].fillColor = self.buttons[cS].get('fillColor')
    ...
    def removebuttons(self):
        if self.buttons != None:
            try:
                for cS in range(len(self.buttons)):
                    self.buttonpolygon[cS].setAutoDraw(False)
                    self.buttonlabels[cS].setAutoDraw(False)
            except:
                booler = 1
                    
    def testparallelport(self):
        self.paralleltriggermodule = True
        if self.paralleltriggermodule:
            try:
                ...
                print('paralleltriggermodule set to false')
